...
static void UartInitNvic(uint8_t u8Idx)
{
    IRQn_Type enIrqIndex;//中断号
    
    ASSERT(IS_VALID_CH(u8Idx));;
    enIrqIndex = (IRQn_Type)(UART0_IRQn + u8Idx);  //6+u8Idx (0x01)  //中断号
    NVIC_ClearPendingIRQ(enIrqIndex);
    NVIC_SetPriority(enIrqIndex,DDL_IRQ_LEVEL_DEFAULT);//#define DDL_IRQ_LEVEL_DEFAULT    3u  
