...
en_result_t Adc_Init(stc_adc_cfg_t* pstcAdcConfig)
{
    ...
    M0P_ADC->CR0_f.CLKSEL = pstcAdcConfig->enAdcClkSel;//ok
    M0P_ADC->CR0_f.SAM = pstcAdcConfig->enAdcSampTimeSel;//ok
    M0P_ADC->CR0_f.SREF = pstcAdcConfig->enAdcRefVolSel;//配置成外部参考电压
