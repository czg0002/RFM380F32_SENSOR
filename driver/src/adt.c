 ...
 * \param   [in]  pstcAdtCHxXCfg   TIMxX端口配置指针
 *
 * \retval  en_result_t  Ok:  配置成功
 * \retval  en_result_t  ErrorInvalidParameter: 无效参数
 */
                                ...
                                stc_adt_CHxX_port_cfg_t* pstcAdtCHxXCfg)
{
    ...
    if ((AdTIM6 < enAdtUnit) || (NULL == pstcAdtCHxXCfg))
    {
        return ErrorInvalidParameter;
    }
...
en_result_t Adt_EnableBrakePort(uint8_t port, stc_adt_break_port_cfg_t* pstcAdtBrkPtCfg)
{
    ...
    u32Val = M0P_ADT0->PTBKS;
    M0P_ADT0->PTBKS = u32Val | (1u<<port);
...
en_result_t Adt_Disable3Cfg(stc_adt_disable_3_cfg_t* pstcAdtDisable3)
{
    ...
    M0P_ADT0->AOSSR_f.BFILTEN = pstcAdtDisable3->bFltEn;
    M0P_ADT0->AOSSR_f.BFILTS = pstcAdtDisable3->enFltClk;
    M0P_ADT0->AOSSR_f.SOFTBK = pstcAdtDisable3->bSwBrk;
...
{
    return M0P_ADT0->AOSSR_f.FBRAKE;
}
...
en_result_t Adt_PwmDitherConfig(stc_adt_pwm_dither_cfg_t* pstcAdtPwmDitherCfg)
{
    ...
    M0P_ADT0->CR_f.DITENS = pstcAdtPwmDitherCfg->enAdtPDType;
    M0P_ADT0->CR_f.DITENB = pstcAdtPwmDitherCfg->bTimxBPDEn;
    M0P_ADT0->CR_f.DITENA = pstcAdtPwmDitherCfg->bTimxAPDEn;
    return Ok;
}
 ...
 *
 * \param   [in]  enAdtUnit           ADV Timer通道选择（TIM4、TIM5、TIM6）
 * \param   [in]  pstcAdtZMaskCfg     计数配置指针
 *
 * \retval  en_result_t  Ok:  配置成功
 * \retval  en_result_t  ErrorInvalidParameter: 无效参数
 */
en_result_t Adt_ConfigZMask(en_adt_unit_t enAdtUnit, stc_adt_zmask_cfg_t* pstcAdtZMaskCfg)
{
    ...
    if ((AdTIM6 < enAdtUnit) || (NULL == pstcAdtZMaskCfg))
    {
        return ErrorInvalidParameter;
    }
